...
describe("chunk persistence", () => {
  it("round-trips an edited chunk's blocks, version, and edited flag", async () => {
    const blocks = sampleBlocks();
    await saveEditedChunk(SEED_A, KEY, blocks, 7);
  ...
  it("stores a copy, not a view of the live buffer", async () => {
    const blocks = sampleBlocks();
    await saveEditedChunk(SEED_A, KEY, blocks, 1);
  ...
  it("isolates worlds: a different seed does not see another's chunk", async () => {
    await saveEditedChunk(SEED_A, KEY, sampleBlocks(), 1);
